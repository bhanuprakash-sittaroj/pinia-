<template src="./guestProfileItem.template.html"></template>
<script>
    import moment from "moment-timezone";
    export default {
        // options
        name: "guestProfileItem",
        props: ["id", "guestDetail"],
        data() {
            return {
                listType: 'list',
                guestProfileList: [],
        ...
        methods: {
            more(id) {
                let vm = this;
                setTimeout(() => {
                    console.log("widthhhhhh", window.innerWidth)
                    if (window.innerWidth <= 1280) {
                        if (document.getElementById(id) && document.getElementById(id).offsetWidth && document.getElementById(id).offsetWidth > 890) {
                            console.log("hhhhh", document.getElementById(id).style)
                            document.getElementById(id).style.overflowX = "hidden";
                            document.getElementById(id).style.overflowY = "hidden";
                            document.getElementById(id).style.width = 895 + 'px';
                            vm.text = "More..";
                        ...
                        if (document.getElementById(id) && document.getElementById(id).offsetWidth && document.getElementById(id).offsetWidth > 1100) {
                            console.log("hhhhh", document.getElementById(id).style)
                            document.getElementById(id).style.overflowX = "hidden";
                            document.getElementById(id).style.overflowY = "hidden";
                            document.getElementById(id).style.width = 1155 + 'px';
                            vm.text = "More..";
                            // console.log('text', text)
                            return vm.text;
                        }
            ...
            createNewInstance(item) {
                ...
                else if(item.AccountType === 7){
                    this.$router.push("/companyProfileDetail/" + (item.GuestProfileId != null || item.GuestProfileId != undefined ? item.GuestProfileId : '-1'));                    
                }                
            },
            ...
            getBalancePriority(details){
                if (details.FinalAmount < details.TotalAmount) {
                     return true;
                   } else {
                     return false;
                   }
        ...
        computed: {
            formattedTitle(){
                if (this.guestDetail.GuestLastName) {        
                 return `${this.guestDetail.GuestFirstName} ${this.guestDetail.GuestLastName}`;
                } else {        
                    return this.guestDetail.GuestFirstName;
                 }
            }
        ...
        mounted() {
            let vm = this;
            console.log("guestDetail--",this.guestDetail);
        }
    }
