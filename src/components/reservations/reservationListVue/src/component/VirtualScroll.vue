...
export default {
  ...
  props: {
    ...
    pulldownText: {
      ...
      default: () => {
        return {
          begin: 'Pull down to refresh',//'下拉刷新'
          trigger: 'Release update',//'释放更新',
          refresh: 'loading...',
          complete: 'update completed',//'更新完成',
          error: 'Update failed',//'更新失败'
        }
      }
    },
    variable: {
  ...
  watch: {
    items (newArr, oldArr) {
      ...
      if (this.pool.length < this.poolLength) {
        ...
        let index = this.items.findIndex((item) => { return item[this.id] === this.pool[0][this.id] })
  ...
  methods: {
    generateItemAccumulator (init) {
      ...
      if (init) {
        ...
        // console.log(" this.accumulator",  this.accumulator,  "item.height   ", item.height)
      } else {
    ...
    updateScrollView () {
      if (!this.variable) {
        ...
        while (i < this.pool.length) {
          ...
          // top = this.pool[i]._top + this.pool[i].height
          if (i < majorLength) {
